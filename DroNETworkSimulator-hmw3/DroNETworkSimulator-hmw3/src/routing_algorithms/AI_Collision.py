...
class AI_Collision_1811110(BASE_routing):
    ...
    def relay_selection(self, opt_neighbors, pkd):
        ...
        if self.drone_path == [] and self.drone.waypoint_history != []:
            if self.drone.waypoint_history[self.drone.current_waypoint - 1][1] < \
                    self.drone.waypoint_history[self.drone.current_waypoint - 2][1]:
            ...
            if self.drone_path.index(self.drone.next_target()) == 0 and self.drone.buffer_length() >= 3:
                return -1
        if self.drone.next_target() == self.simulator.depot.list_of_coords[0] or self.drone.next_target() == \
            ...
            if self.drone_not_seen([v[1] for v in opt_neighbors], pkd):
                ...
                if self.already_chosen_check(pkd):
                    ...
                    if give_packet_away:
                        ...
                        self.packet_generation = [x for x in self.packet_generation if
                                                  x[0].event_ref.identifier != pkd.event_ref.identifier]
                        self.packet_generation = sorted(self.packet_generation, key=lambda x: x[1])
                    else:
    ...
    def q_back_feedback(self, drone):
        ...
        if self.drone.buffer_length() != 0 and drone == self.drone and self.state_action_list != []:
            ...
            for state, action_index, step in reversed(self.state_action_list):
                ...
                self.q_dict[state][action_index] = self.q_dict[state][action_index] + self.alpha * \
                                                   (reward + self.gamma * max(self.q_dict[future_state]) -
                                                    self.q_dict[state][action_index])
    ...
    def make_choice(self, choice, cell_index, pkd):
        if choice:
            self.taken_actions[pkd.event_ref.identifier][len(self.taken_actions[pkd.event_ref.identifier]) - 1] = (
                (cell_index, True, -1), self.simulator.cur_step)
        else:
            self.taken_actions[pkd.event_ref.identifier][len(self.taken_actions[pkd.event_ref.identifier]) - 1] = (
                (cell_index, False, None), self.simulator.cur_step)
    ...
    def already_chosen_check(self, pkd):
        if len(self.taken_actions[pkd.event_ref.identifier]) == 0:
            return False
        if self.taken_actions[pkd.event_ref.identifier][len(self.taken_actions[pkd.event_ref.identifier]) - 1][0][
            1] == True:
            return True
        return False
    ...
    def update_next_state(self, cell_index):
        ...
        for pkd in self.taken_actions.keys():
            ...
            if len(past_actions) == 0:
                pass
            if past_actions[len(past_actions) - 1][0][1] == False and past_actions[len(past_actions) - 1][0][
                0] != current_state:
                past_actions[len(past_actions) - 1] = (
                    (past_actions[len(past_actions) - 1][0][0], past_actions[len(past_actions) - 1][0][1],
                     current_state),
                    past_actions[len(past_actions) - 1][1])
    ...
    def correct_trasmission_error(self, pkd, cell):
        ...
        past_actions[len(past_actions) - 1] = (
            (past_actions[len(past_actions) - 1][0][0], False, cell), past_actions[len(past_actions) - 1][1])
    ...
    def is_coming_back(self):
        return self.drone.next_target() == self.simulator.depot.list_of_coords[0] or self.drone.next_target() == \
               self.simulator.depot.list_of_coords[1]
